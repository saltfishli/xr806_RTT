...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
...
#if XRADIO
	bool state_disconnected_change = false;
#endif
	...
	case BT_CONN_DISCONNECTED:
		...
		bt_conn_ref(conn);
#if XRADIO
		state_disconnected_change = true;
#endif
		break;
	case BT_CONN_CONNECT:
		if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
	...
	case BT_CONN_DISCONNECT:
		...
		if (old_state == BT_CONN_DISCONNECT_COMPLETE || old_state == BT_CONN_DISCONNECTED) {
			...
			/*if BT_CONN_DISCONNECT_COMPLETE -> BT_CONN_DISCONNECTED then
			BT_CONN_DISCONNECTED -> BT_CONN_DISCONNECT, old state will be
			BT_CONN_DISCONNECTED, then conn ref will increase in this function,
			but it does not decrease here, so the ref will not be zero after
			disconnect command complete, so we should decrease conn ref here to
			make it right, add by liuzhang, 20210425*/
			if (state_disconnected_change) {
				bt_conn_unref(conn);
			}
		}
#endif
